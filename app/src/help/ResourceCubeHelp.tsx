  ...
  return (
    <>
      ...
      {resource === Resource.Krystallium ? (
        <>
          <p css={associationCss}>{t('help.krystallium.persist', 'Unlike other resource cubes, Krystallium is kept from one round to the next.')}</p>
          <h3 css={sectionTitleCss}>{t('help.resource.obtain', 'How to obtain')}</h3>
          <ul>
            <li>
              {t('help.krystallium.obtain.conversion', 'Every 5 resource cubes placed on your Empire card are automatically converted into 1 Krystallium.')}
            </li>
          ...
          <p>
            {t(
              'help.krystallium.use',
              'Krystallium is a wild resource: it can replace any resource cube when paying the construction cost of a Development. However, it cannot replace a Character token.'
            )}
          </p>
        </>
      ) : (
        <>
          ...
          <ul>
            <li>
              {t(
                'help.resource.obtain.recycling',
                'Recycling: during the Planning phase, discarding a card from your Draft area gives you a resource cube as a recycling bonus.'
              )}
            </li>
            <li>
              {t(
                'help.resource.obtain.production',
                'Production: during the Production phase, each player produces resources based on the symbols on their Empire card and their built Developments.'
              )}
            </li>
          </ul>
          <h3 css={sectionTitleCss}>{t('help.resource.use', 'How to use')}</h3>
          <ul>
            <li>
              {t(
                'help.resource.use.construction',
                'Place them on Development cards Under Construction to pay their construction cost. Each cost slot requires a specific resource type.'
              )}
            </li>
            <li>
              {t(
                'help.resource.use.empire',
                'Place them on your Empire card. Every 5 resource cubes on the Empire card are automatically converted into 1 Krystallium.'
              )}
            </li>
          </ul>
        </>
